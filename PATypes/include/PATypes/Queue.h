...
namespace PATypes {
	...
	T Queue<T>::pop() {
		if (list->getLength() == 0)
			throw std::out_of_range("при попытке забрать элемент из Queue Queue пустая");
		T res = list->getFirst();
		if (list->getLength() > 1) {
	...
	template<class T>
	Sequence<T> *Queue<T>::append(T item) {
	...
	template<class T>
	Sequence<T> *Queue<T>::concat(Sequence<T> *list) {
		return this->list->concat(list);
	}

	template<class T>
	Sequence<T> *Queue<T>::map(T (*f)(T)) {
		return list->map(f);
	}
	...
	template<class T>
	Sequence<T> &Queue<T>::operator+(Sequence<T>& sequence) {
		return (*list) + sequence;
	}

	template<class T>
	IEnumerator<T> *Queue<T>::getEnumerator() {
		return list->getEnumerator();
	}

	template<class T>
	Sequence<int> *Queue<T>::findSequence(Sequence<T>* toFind) {
		int toFindIndex = 0;
		int stackIndex = 0;
