...
namespace PATypes {
	...
	template<class T>
	SegmentedList<T>::SegmentedList(LinkedListNode<DynamicArray<T>> *start, int count) : size(count) {
		...
		for (int i = 1; i < count; ++i) {
			...
			intermediate = new LinkedListNode<DynamicArray<T>>(currentSource->getNext()->get());
			current->setNext(intermediate);
			current = current->getNext();
		}
	}
	...
	template<class T>
	SegmentedList<T>::SegmentedList(const SegmentedList<T> &list) : size(list.size) {
	...
	template<class T>
	void SegmentedList<T>::append(T item) {
		this->size++;
		if (this->head)
	...
	template<class T>
	void SegmentedList<T>::prepend(T item) {
		this->size++;
	...
	template<class T>
	void SegmentedList<T>::insertAt(T item, int index) {
		try {
	...
	SegmentedList<T> *SegmentedList<T>::concat(SegmentedList<T> *list) {
		...
		while (currentOriginal->getNext()) {
			current->setNext(new LinkedListNode<T>(currentOriginal->getNext()->get()));
			current = current->getNext();
	...
	template<class T>
	void SegmentedList<T>::map(T (*f)(T)) {
	}
